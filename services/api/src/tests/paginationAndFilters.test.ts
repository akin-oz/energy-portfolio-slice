...
type ProjectsQueryData = {
  projectsByCustomer: {
    edges: Array<{
      cursor: string;
      node: { id: string; status: string; customerId: string; createdAt: string };
    }>;
...
type AssetsQueryData = {
  energyAssetsByProject: {
    edges: Array<{
      cursor: string;
      node: { id: string; type: string; projectId: string; createdAt: string };
    }>;
    pageInfo: { endCursor: string | null; hasNextPage: boolean };
  };
};
...
describe("GraphQL pagination and filters", () => {
  ...
  it("paginates energy assets by project with type filter", async () => {
    ...
    try {
      ...
      const data = getData<AssetsQueryData>(res);
